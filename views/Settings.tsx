...
const Settings: React.FC = () => {
  const { settings, setSettings, setCrops, setTransactions, setTodos, crops, transactions, todos, triggerUIInteraction } = useFarm();
        ...
        <Card title="Farm Details">
            <div className="space-y-4">
                <div>
                    ...
                    <input type="text" value={settings.farmName} onChange={e => setSettings(p => ({...p, farmName: e.target.value}))} className="mt-1 block w-full p-3 border rounded-lg text-lg" onFocus={() => triggerUIInteraction("Set the name of your farm.")} onBlur={clearHint} />
                </div>
                <div>
                    <label className="block text-lg font-medium text-text-secondary">Currency</label>
                    <input type="text" value={settings.currency} onChange={e => setSettings(p => ({...p, currency: e.target.value}))} className="mt-1 block w-full p-3 border rounded-lg text-lg" onFocus={() => triggerUIInteraction("Set the currency code (e.g., USD, EUR).")} onBlur={clearHint} />
                </div>
            </div>
        </Card>
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <div>
                    ...
                    <div className="flex mb-4">
                        <input type="text" value={newIncomeCat} onChange={e => setNewIncomeCat(e.target.value)} placeholder="New income source" className="flex-grow p-2 border rounded-l-lg" onFocus={() => triggerUIInteraction("Type a new category for your income.")} onBlur={clearHint} />
                 ...
                 <div>
                    ...
                    <div className="flex mb-4">
                        <input type="text" value={newExpenseCat} onChange={e => setNewExpenseCat(e.target.value)} placeholder="New expense category" className="flex-grow p-2 border rounded-l-lg" onFocus={() => triggerUIInteraction("Type a new category for your expenses.")} onBlur={clearHint} />
        ...
        <Card title="Data Management">
            <div className="flex items-center space-x-4">
                <button 
                  ...
                  onMouseEnter={() => triggerUIInteraction("Download all your farm data to a file.")}
                ...
                <div>
                    <label 
                      ...
                      className="cursor-pointer px-6 py-3 rounded-lg text-white bg-yellow-500 hover:bg-yellow-600"
                      onMouseEnter={() => triggerUIInteraction("Restore data from a backup file. This will overwrite current data.")}
                      onMouseLeave={clearHint}
                    >
                      Restore Data
                    </label>
